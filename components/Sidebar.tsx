        ...
        <aside className="w-64 bg-white text-gray-800 flex-col flex-shrink-0 hidden sm:flex border-r">
            ...
            <nav className="flex-1 px-4 py-6 space-y-2">
                {navLinks.map(link => (
                    <NavLink
                        ...
                        className={({ isActive }) =>
                            ...
                            isActive ? 'bg-primary-100 text-primary-700' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'
                            }`
                        }
                    >
                        {link.icon}
                ...
                {placeholderLinks.map(link => (
                    <div key={link.label} className="flex items-center px-4 py-3 rounded-xl font-medium text-gray-400 cursor-not-allowed">
                       {link.icon}
                       <span className="ml-4">{link.label}</span>
                    </div>
                ))}
            ...
            <div className="px-4 py-6 border-t">
                <button
                    ...
                    className="w-full flex items-center px-4 py-3 rounded-xl font-medium text-gray-500 hover:bg-red-50 hover:text-red-600 transition-colors duration-200"
