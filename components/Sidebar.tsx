...
const Sidebar: React.FC = () => {
    ...
    return (
        <aside className="w-64 bg-black/20 backdrop-blur-lg text-white flex-col flex-shrink-0 hidden sm:flex">
            ...
            <nav className="flex-1 px-4 py-6 space-y-2">
                ...
                {placeholderLinks.map(link => (
                    <div key={link.label} className="flex items-center px-4 py-3 rounded-xl font-medium text-primary-400 cursor-not-allowed opacity-60">
                       {link.icon}
                       <span className="ml-4">{link.label}</span>
                    </div>
                ))}
            ...
            <div className="px-4 py-6 border-t border-white/10">
                <button
                    ...
                    className="w-full flex items-center px-4 py-3 rounded-xl font-medium text-primary-200 hover:bg-white/10 transition-colors duration-200"
